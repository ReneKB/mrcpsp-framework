...
@Service("UncertaintyProactiveReactiveExperiment")
@Log4j2
public class UncertaintyProactiveReactiveExperiment extends UncertaintyExperiment {
    ...
    public Schedule buildUncertaintySolution(Schedule plannedSolution, Benchmark benchmark, Solver solver, int iterations, Metric<?> robustnessFunction, UncertaintyModel uncertaintyModel) {
        ...
        while (buildingActivityModeList.size() < plannedActivityModeList.size()) {
            ...
            try {
                potentialSchedule = solver.algorithm(subbenchmark,
                        Math.min(iterations / 2, 2000), ScheduleComparator.builder().primary(Metrics.COST).secondary(Metrics.MAKESPAN).build(),
                        buildingActivityModeList, plannedSolution);
