...
import de.uol.sao.rcpsp_framework.model.*;
...
public class BenchmarkLoaderService {
    ...
    enum CurrentSegmentState {
        ...
        public static CurrentSegmentState getValue(int index) {
            return CurrentSegmentState.values()[index];
        }
    }

    @SneakyThrows
    public Benchmark loadBenchmark(String file) {
        InputStream inputStream = BenchmarkLoaderService.class.getClassLoader().getResourceAsStream(file);
        ...
        while ((strLine = br.readLine()) != null) {
            ...
            switch (CurrentSegmentState.getValue(currentSegmentStateIndex)) {
                case Header:
                    break;
                case ProjectMeta1:
